import shutil
from script.my_util import *
import os
import pandas as pd
import subprocess

save_dir = "../datasets/preprocessed_data/"
save_no_comment_dir = "../datasets/preprocessed_data_no_comment/"
sourcecode_dir = "../sourcecode/"
# The following paths need to be replaced
graph_tool_path = r'E:\OpenSourceCode\PropertyGraph-main\out\artifacts\PropertyGraph_jar\PropertyGraph.jar'

if not os.path.exists(sourcecode_dir):
    os.makedirs(sourcecode_dir)

if not os.path.exists(save_no_comment_dir):
    os.makedirs(save_no_comment_dir)


def remove_comments_and_blank_lines(proj_name):
    cur_all_rel = all_releases[proj_name]
    for rel in cur_all_rel:
        df_rel = pd.read_csv(os.path.join(save_dir, f'{rel}.csv'), na_filter=False)
        df_filtered = df_rel[~(df_rel['is_comment'] | df_rel['is_blank'])].copy()
        df_filtered['line_number'] = df_filtered.groupby('filename').cumcount() + 1
        df_filtered.reset_index(drop=True, inplace=True)
        df_filtered.to_csv(os.path.join(save_no_comment_dir, f'{rel}.csv'), index=False)
        print(f'{rel} done')


def preprocess_code_line(code_line):
    code_line = code_line.strip()
    return code_line


def get_sourcecode(proj_name):
    cur_all_rel = all_releases[proj_name]

    for rel in cur_all_rel:
        df_rel = pd.read_csv(os.path.join(save_no_comment_dir, f'{rel}.csv'), na_filter=False)
        df_rel['code_line'] = df_rel['code_line'].apply(preprocess_code_line)

        grouped = df_rel.groupby('filename')
        for filename, group in grouped:
            code = '\n'.join(map(str, group['code_line']))
            output_path = os.path.join(f'{sourcecode_dir}/{rel}', filename)
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
            with open(output_path, 'w', encoding='utf-8') as f:
                f.write(code)
        print(f'{rel} done')


def get_PDG_dot(proj_name):
    cur_all_rel = all_releases[proj_name]

    for rel in cur_all_rel:
        df_rel = pd.read_csv(os.path.join(save_no_comment_dir, f'{rel}.csv'), na_filter=False)
        grouped = df_rel.groupby('filename')
        for filename, group in grouped:
            java_path = f'{sourcecode_dir}/{rel}/{filename}'
            get_pdg_command = f'java -jar {graph_tool_path} -d {java_path} -p'
            pdg_result = subprocess.run(get_pdg_command, shell=True, capture_output=True, text=True)
            if pdg_result.returncode == 0:
                java_dir = os.path.dirname(java_path)
                pdg_dir = os.path.join(java_dir, 'PDG')
                move_files(pdg_dir, java_dir)
                os.rmdir(pdg_dir)
            else:
                print('error:', java_path)
        print(f'{rel} done')


def move_files(src_dir, dst_dir):
    files = os.listdir(src_dir)

    src_file = os.path.join(src_dir, files[0])
    dst_file = os.path.join(dst_dir, files[0])
    shutil.move(src_file, dst_file)


if __name__ == '__main__':
    for proj in list(all_releases.keys()):
        remove_comments_and_blank_lines(proj)
        get_sourcecode(proj)
        get_PDG_dot(proj)
