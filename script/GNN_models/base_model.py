...
from GNN_models.hgt import HGT
from GNN_models.pna import PNA
...
class STnet(nn.Module):
    def __init__(self, nfeat, nhid, nclass, gnn, nlayers=2, gat_heads=4, dropout=0.5, with_bn=True, with_bias=True):
        ...
        if gnn == "GCN":
            ...
            self.gnn_model = HGT(nfeat, nhid, nclass, num_heads=gat_heads, num_layers=nlayers,
                                 dropout=dropout, with_bn=with_bn, with_bias=with_bias)
        else:
    ...
    def forward(self, x, edge_index, batch):
        ...
        if self.gnn_model.name.startswith("HGT"):
            x_dict = {"node": x}
            edge_index_dict = {("node", "edge", "node"): edge_index}
            node_emb = self.gnn_model(x_dict, edge_index_dict)
        else:
        ...
        graph_pred = self.classifier(graph_rep)
        node_pred = self.node_classifier(node_emb)

        return graph_pred, node_pred, graph_rep
...
class Tenet(nn.Module):
    def __init__(self, nfeat, nhid, nclass, gnn, nlayers=2, gat_heads=4, dropout=0.5, tau=0.1, with_bn=True,
        ...
        if gnn == "GCN":
            ...
            self.gnn_model = HGT(nfeat, nhid, nclass, num_heads=gat_heads, num_layers=nlayers, dropout=dropout,
    ...
    def forward(self, x, y, edge_index, batch):
        ...
        if self.gnn_model.name.startswith("HGT"):
            x_dict = {"node": x}
            edge_index_dict = {("node", "edge", "node"): edge_index}
            fea = self.gnn_model(x_dict, edge_index_dict)
        else:
